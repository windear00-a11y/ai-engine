"""Filesystem safety and path-validation helpers for the trust layer.

All gated tools resolve paths through a :class:`Workspace` so that paths can
never escape the configured root. This is the single place where path-trust
decisions are made; individual tools never call ``os.path`` joining on raw
user input directly.

This is generic Persistent Intelligence Core infrastructure (path trust). It
was formerly located under the removed coding domain; it is promoted here
because the Core trust layer (PathPolicy) depends on it.
"""
