"""Lightweight structural types (dicts) returned by the SDK client.

The SDK keeps the engine's plain, JSON-serializable structures instead of
inventing a heavy object graph. TypedDicts give users editor/type-checker
help without wrapping or copying the data the engine already returns.

These mirror the documented response shapes of the tool contract
(:mod:`api.contract`) and nothing else. If the engine adds an optional field
later, the underlying dict still carries it; the types here only describe the
stable, documented keys.
"""
